---
name: hypt-start
description: "Onboards a new project by clarifying the idea, setting up accounts, and creating a build plan. Use when the user wants to start a new app or project."
metadata:
...
The SQL templates are bundled with this skill under `assets/`. Resolve that directory relative to this `SKILL.md` before copying them:

```bash
ASSETS_DIR="<absolute path to this skill>/assets"
```
...
> - Ask your agent to commit, push, review, test, or fix the current work normally
>
> If you want to start fresh with a new idea, rename or delete the existing plan file and use `hypt-start` again.
...
Run read-only checks silently before proposing any installation. Detect the operating system, architecture, available package manager, installed versions, and authentication state. Use the current shell's equivalent commands; these POSIX examples are illustrative:

```bash
uname -s
uname -m
command -v brew || command -v winget || command -v apt-get || command -v dnf
git --version
node --version
npm --version
npx --version
...
Also inspect the project without printing secrets:

```bash
test -f package.json
test -f .env.local
test -e vercel.json -o -d .vercel
```

Do not invoke `npx` or `bunx` merely to check a missing command: package runners may download code. First finish the installation plan and get consent.

#### Step 3b: Present and execute one tool plan

Build one table containing every missing or outdated prerequisite:

| Tool | Why it is needed | Current state | Approved source | Proposed change | Admin/restart needed |
|---|---|---|---|---|---|
| Git | Version control | ... | OS package manager or git-scm.com | ... | ... |
| Node.js LTS + npm/npx | Agent Skills installer | ... | OS package manager or nodejs.org | ... | ... |
| GitHub CLI | Repository and PR workflow | ... | OS package manager or cli.github.com | ... | ... |
| Bun | Default project toolchain | ... | OS package manager or bun.sh | ... | ... |

Include only tools the selected workflow needs. Vercel and Supabase CLIs should run on demand through `bunx`; do not install provider CLIs globally without a concrete reason.

Explain:

> These are mainstream developer tools commonly used by coding agents. I will use your operating system's package manager or the vendor's official distribution, show the planned changes, and verify every tool afterward. You may see a normal administrator-password or browser sign-in prompt; enter credentials only into that system prompt or official browser page, never into chat.

Ask once for approval of the complete installation plan. After approval, install every approved item in one pass using current official instructions. If a package manager itself is missing, include its installation in the plan rather than adding it silently. Avoid downloaded-script-to-shell installers when a signed package or system package manager is available.

Verify every installed command and report any item that still needs a restart or manual action. If every core tool is already in place, say:

> Your core developer tools are already installed. Next I'll check only the account connections this project needs.

#### Step 3c: Account signup and authentication

Present only the accounts required by the chosen project. GitHub comes first because other services commonly support GitHub OAuth. Each browser authentication requires its own user interaction, but the CLI installations should already be complete.
...
**Vercel** (when selected for hosting and `bunx vercel whoami` fails)
...
**Supabase** (when selected for data/auth and `bunx supabase projects list` fails)
...
> 2. Once you're in the dashboard, open **Developers → API keys**
> 3. Keep this chat open, but do not paste either key here. I'll prepare a gitignored local environment file and show you where to enter the test keys directly on your computer.

Before secret entry, use `git check-ignore .env.local` to verify that `.env.local` cannot be committed. If it is not ignored, add it to `.gitignore` and verify again. Create named placeholders, ask the user to replace them in their local editor, and verify only whether the variables are present—never print their values.
...
> 4. Do not paste the key into chat. Enter it in the gitignored local environment file I prepared.
...
After CLI authentication, mention optional AI-app OAuth connections:

> Your local developer tools are connected. Claude and ChatGPT can also connect to services such as Gmail, Google Drive, GitHub, and others through their Connectors, Apps, or Plugin settings. Those connections are optional and separate from the CLI logins. Connect only services this project needs, review the requested read/write permissions, and use the official OAuth page rather than sharing credentials in chat.

Name the relevant services based on the project answers and point the user to the current host app's connection settings. If a service is unavailable there, continue with its official CLI or website.

#### Step 3d: Project scaffolding and environment wiring (default stack)
...
mkdir -p supabase/migrations
cp "$ASSETS_DIR/integrations.sql" \
...
>
> When you want the latest hypt workflows, ask your coding agent to run `npx skills update -g`. Updates remain explicit so you can review when your installed instructions change.
...
Each item is a small task. `hypt-prototype` will work through them after the core features are built.
```
...
> Anytime: ask me to save work, review changes, check the site, or suggest what to do next.

Then a single **Extras** line when optional companion skills are available:

- If gstack skills are available: `cso` (security audit), `office-hours` (rethink product), `design-review` (visual polish)
- If `grill-me` is available: `grill-me` (stress-test plan)

Omit the Extras line when no optional companion skills are available.
...
After the project is scaffolded, verified, and the build plan is written, ask whether the user wants lightweight CI. If yes, add one GitHub Actions workflow that installs dependencies from the lockfile and runs the repository's existing lint, typecheck, and test commands. Do not invent test infrastructure or duplicate checks the repository already runs.
