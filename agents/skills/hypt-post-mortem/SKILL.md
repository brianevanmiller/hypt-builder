...
This skill runs after `hypt-restore` to document what went wrong. It is designed for non-technical users — write in plain language, avoid jargon, and focus on what happened and what to do next.
...
**If the root cause is classified as "involved" and gstack's `investigate` skill is unavailable:**
...
> **Note:** The root cause couldn't be definitively determined from the code diff alone. For deeper investigation, install gstack using its current installation instructions and use its `investigate` skill in a new session.
...
Start a new coding-agent session and use one of:

- `hypt-go` with "fix the most recent post-mortem issue" — does the research, plans, builds, and reviews, then asks before merging
- `hypt-yolo` with "fix the most recent post-mortem issue" — fully autonomous fix and ship
- Or ask the agent normally to diagnose and fix the bug with more control over each step
```
...
> **Want to fix this?** Start a new agent session and say:
>
> - `fix the most recent post-mortem issue` — the agent will diagnose and fix the bug step by step
>
> Or for more automation:
> - `hypt-go` with "fix the most recent post-mortem issue" — autonomous fix with confirmation before merge
> - `hypt-yolo` with "fix the most recent post-mortem issue" — fully autonomous fix and ship
>
> The post-mortem document has all the details the agent needs to understand what went wrong.
