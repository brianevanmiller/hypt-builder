...
Identify concrete follow-up work revealed by the branch. Add non-duplicate, actionable items to the project's existing backlog when one exists; otherwise include them in the final summary without creating a new tracking system.
...
**Update version files when the project has them:**
...
> **CI suggestion:** Now that [feature] is live, it'd be worth adding [specific test type] to CI. This would catch [specific risk] automatically. Want me to set that up? (Use `hypt` and ask to set up CI in a new workspace.)
